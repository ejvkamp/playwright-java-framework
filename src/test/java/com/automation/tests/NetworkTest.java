...
public class NetworkTest extends BaseTest {
	...
	public void verifyOfflineHandling() {
		...
		try {
			...
			try {
				...
				// Expected behavior: Browser throws a network error because we are offline
	...
	public void verifyNetworkRecoveryIndicators() {
		...
		try {
			...
			// The red banner hides and green "Online" banner should appear
	...
	public void verifySlowNetworkHandling() {
		// As CDP requires Chromium skip on unsupported browsers
		if (!"chromium".equalsIgnoreCase(ConfigReader.getProperty("browser"))) {
			throw new SkipException("Skipping Test: CDP network throttling requires Chromium.");
		}
		...
		try {
			...
			// Assertion: It should take longer than a normal load (e.g. > 2s)
			if (duration < 2000) {
